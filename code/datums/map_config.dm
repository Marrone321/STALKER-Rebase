/datum/map_config
	...
	var/list/job_listings = list(/datum/job_listing/stalker)
...
/datum/map_config/proc/get_map_info()
	return "Embark into The Zone!"

/proc/load_map_config(filename = "data/next_map.json", default_to_box, delete_after, error_if_missing = TRUE)
	var/datum/map_config/config
	if (default_to_box)
		config = new /datum/map_config/ZonaRemastered()
		return config
	config = LoadConfig(filename, error_if_missing)
	if (!config)
		config = new /datum/map_config/ZonaRemastered()  // Fall back to Box
	if (delete_after)
		fdel(filename)
	return config
